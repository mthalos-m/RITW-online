/* ============================================================
   ...
   Sections: rendered Mermaid diagram · text from the book ·
...
(function () {
    ...
    document.title = `Protocol ${protocol.id}: ${protocol.title} — Reasoning in the Wild`;

    /* ---------- prev / next ---------- */
    const prev = protocols[idx - 1];
    const next = protocols[idx + 1];
    prevNext.innerHTML =
        (prev ? `<a href="protocol.html?id=${encodeURIComponent(prev.id)}">&#8592; ${escHtml(prev.id)}</a>` : "") +
        (prev && next ? `<span style="color:var(--rule);margin:0 0.5rem">|</span>` : "") +
        (next ? `<a href="protocol.html?id=${encodeURIComponent(next.id)}">${escHtml(next.id)} &#8594;</a>` : "");

    /* ---------- chapter label ---------- */
    const ch = (typeof chapters !== "undefined") && chapters[protocol.chapter];
    const chLabel = ch ? `${ch.label} — ${ch.title}`
                       : (protocol.userContributed ? "Contributed protocol" : `Chapter ${protocol.chapter}`);

    /* full Mermaid source, if this protocol has a rebuilt diagram */
    ...
    wrap.innerHTML = `
        <header class="detail-header" style="border-left:3.5px solid var(--ch${protocol.chapter}, var(--accent));padding-left:1.2rem;">
            ...
            <div class="detail-meta">
                <span>Protocol ${escHtml(protocol.id)}</span>
        ...
        <section class="detail-section">
            ...
            <figure class="detail-figure" id="diagram-figure">
                <div id="diagram-mermaid">${ hasMermaid ? `<div class="loading-note">Rendering diagram…</div>` : "" }</div>
                <div id="diagram-original" class="hidden">
                    ${ protocol.image
                        ? `<img src="${escHtml(protocol.image)}" alt="Original figure for Protocol ${escHtml(protocol.id)}">`
        ...
        <!-- WALKTHROUGH -->
        ${ (hasMermaid && Array.isArray(protocol.walkthrough) && protocol.walkthrough.length) ? `
        <section class="detail-section">
    ...
    if (hasMermaid) {
        renderInto("diagram-mermaid", fullCode, "main-diagram");
    }
    ...
    if (hasMermaid && Array.isArray(protocol.walkthrough) && protocol.walkthrough.length) {
        initWalkthrough(protocol);
    }
    ...
    function initWalkthrough(p) {
        ...
        /* progress dots */
        progress.innerHTML = steps.map((_, n) =>
            `<span class="walk-dot" data-n="${n}"></span>`).join("");
        ...
        function show() {
            ...
            /* code panel: show every line, highlight the ones active at this step */
